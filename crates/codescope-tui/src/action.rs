...
//!
//! codescope is read-only, so there is a single "normal" mode. `j`/`k` and arrow keys support
//! vertical navigation; horizontal diff movement uses only Left/Right. `?` opens the help modal
//! (the discovery path). [`map_key`] is pure so the entire keymap is unit-testable without a
//! terminal (research 04 §4, §6).
...
/// One action received from an external `codescope agent` client.
///
/// The structured envelope survives the socket-to-TUI hop so telemetry can correlate the
/// short-lived client session with the exact visible mutation it caused. The dispatcher still
/// receives the ordinary typed [`Action`].
#[derive(Debug, Clone, PartialEq)]
pub struct ExternalControl {
    /// Globally unique identity generated by the CLI process for this command.
    pub command_id: String,
    /// Stable protocol operation such as `context`, `focus`, or `diagram.edit`.
    pub operation: String,
    /// Captured view identity for view-bound operations.
    pub view_id: Option<String>,
    /// Typed action to apply through the ordinary TUI/dispatcher path.
    pub action: Action,
}
...
pub enum Action {
    ...
    /// Inspect a diagram by captured view identity without changing the visible selection.
    AgentDiagramInspect {
        /// Identity returned through the published snapshot after the dispatcher applies it.
        request_id: u64,
        /// Repository epoch embedded in the external view identifier.
        epoch: codescope_core::Epoch,
        /// Exact selection captured by `codescope agent context`.
        selection: crate::snapshot::AiSummaryKey,
    },
    ...
    AgentDiagram {
        ...
        command: Box<codescope_core::DiagramCommand>,
    },
    ...
    ToggleExpand,
    /// Toggle the explicit review mark on the selected directory, file, or LSP object.
    ToggleReviewed,
    ...
    ToggleWrap,
    /// Move the unwrapped diff horizontally by a signed number of display cells. Keyboard
    /// Left/Right and horizontal trackpad scrolling share this exact action.
    ScrollDiffHorizontal {
        /// Negative moves toward the line start; positive moves toward the line end.
        delta: i16,
    },
...
pub fn map_key(key: KeyEvent, app: &App) -> Action {
    ...
    match key.code {
        ...
        KeyCode::Left if app.focused == Pane::Diff && !app.diff_wrap && app.diff_hscroll > 0 => {
            Action::ScrollDiffHorizontal { delta: -8 }
        }
        KeyCode::Right if app.focused == Pane::Diff && !app.diff_wrap => {
            Action::ScrollDiffHorizontal { delta: 8 }
        }
        KeyCode::Left | KeyCode::Right if app.focused == Pane::Diff => Action::None,
...
mod tests {
    ...
    fn tree_keys() {
        ...
        assert_eq!(map_key(key(KeyCode::Char('h')), &app()), Action::None);
        assert_eq!(map_key(key(KeyCode::Char('l')), &app()), Action::None);
        assert_eq!(map_key(key(KeyCode::Left), &app()), Action::Collapse);
        assert_eq!(map_key(key(KeyCode::Right), &app()), Action::Expand);
    }

    #[test]
    fn diff_horizontal_scroll_uses_arrow_keys_only() {
        let mut diff = app();
        diff.focused = Pane::Diff;
        diff.diff_hscroll = 8;
        assert_eq!(
            map_key(key(KeyCode::Left), &diff),
            Action::ScrollDiffHorizontal { delta: -8 }
        );
        assert_eq!(
            map_key(key(KeyCode::Right), &diff),
            Action::ScrollDiffHorizontal { delta: 8 }
        );
        assert_eq!(map_key(key(KeyCode::Char('h')), &diff), Action::None);
        assert_eq!(map_key(key(KeyCode::Char('l')), &diff), Action::None);

        diff.diff_hscroll = 0;
        assert_eq!(map_key(key(KeyCode::Left), &diff), Action::None);
        diff.diff_wrap = true;
        assert_eq!(map_key(key(KeyCode::Right), &diff), Action::None);
    }
